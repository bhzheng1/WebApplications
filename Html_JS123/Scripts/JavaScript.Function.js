...
//函数表达式(Function Expression),匿名函数，lambda expression
{
    ...
    //命名的函数表达式仍然是表达式
    let sayHello = function func(who) { //名字func为了便于函数内部使用
        if (who) {
...
//函数的任意个数参数
{
    //Rest参数
    {
        function showName(firstName, lastName, ...titles) {
            ...
            for (let title of titles) { //遍历titles参数
                console.log(title);
            }
            for (let arg of arguments) { //遍历所有参数，arguments为内置属性
                console.log(arg);
            }
        }
...
//解构-智能函数参数
{
    ...
    function showMenu({ title = "Untitled", width: w = 200, height = 100, items = [], items: [item1, item2] }) {
...
//函数的属性
{
    ...
    sayHi.hello = function () { //function的静态方法
        console.log("hello");
    };
...
//装饰器函数
{
    //利用缓存装饰器把函数包装一下再返回函数
    {
        ...
        function cachingDecorator(func) {//利用缓存装饰器把函数包装一下再返回函数
            let cache = new Map();
            return function (x) {
                if (cache.has(x)) {
                    return cache.get(x);
...

{
    ...
    // deep copy
    secUpdated = {...person, address:{...person.address,city:"Boston"},name:'Bob',age:22}
